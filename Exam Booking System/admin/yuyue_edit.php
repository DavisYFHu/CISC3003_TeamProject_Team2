...
if (!empty($_GET['id'])) {
    ...
    $sql = "select * from yuyue where id = {$id}";
    // perform a search
    $state = $pdo->query($sql); 
...
if ($_POST) {
    ...
    $sql = "UPDATE yuyue SET name='{$name}', km='{$km}', tel='{$tel}', pxdate='{$pxdate}' WHERE id=$id";
    // Execute sql
    $row = $pdo->exec($sql);
    // Determine whether or not it was successful!
    if ($row) {
        echo "<script>alert('Edit Successfully！');window.location.href='agree_yuyue.php'</script>";
    } else {
        echo "<script>alert('Edit Failed！');window.location.href='agree_yuyue.php'</script>";
    }
}
?>
<!DOCTYPE html>
...
<div class="layui-layout layui-layout-admin">
    ...
    <div class="layui-body">
        ...
        <div class="layui-container">
            <br><br>
            <blockquote class="layui-elem-quote">Edit Booking Information</blockquote>
            <div class="layui-row">
                <div class="layui-col-md8 layui-col-md-offset1">
                    <form class="layui-form layui-form-pane" action="yuyue_edit.php" method="post">
                        ...
                        <div class="layui-form-item">
                            ...
                            <div class="layui-input-block">
                                <input type="text" name="pxdate" id="pxdate" value="<?php echo $result['pxdate']; ?>" required autocomplete="off" placeholder="Please select time" class="layui-input">
                            </div>
                        </div>
                        <div class="layui-form-item">
